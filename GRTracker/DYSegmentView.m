...
-(void)setTitleArray:(NSArray *)titleArray{
    ...
    self.bottomLine = [[UIView alloc]initWithFrame:CGRectMake((self.defaultSelectIndex-1)*(self.segmentWidth/titleArray.count), self.segmentHeight-2, self.segmentWidth/titleArray.count, 2)];
    self.bottomLine.backgroundColor = [UIColor redColor];
    [self.bgScrollView addSubview:self.bottomLine];
    
    self.bgScrollView.contentSize = CGSizeMake(self.segmentWidth, self.segmentHeight);
    
    for (int i = 0; i<_titleArray.count; i++) {
        UIButton *btn = [UIButton buttonWithType:UIButtonTypeCustom];
        btn.frame = CGRectMake(self.segmentWidth/_titleArray.count*i, 0, self.segmentWidth/_titleArray.count, self.segmentHeight-2);
        btn.tag = i+1;
