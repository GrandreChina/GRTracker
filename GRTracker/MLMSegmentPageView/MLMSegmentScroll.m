...
- (void)addViewCacheIndex:(NSInteger)index {
    ...
    if ([object isKindOfClass:[NSString class]]) {
        Class class1 = NSClassFromString(object);
        if ([class1 isSubclassOfClass:[UIViewController class]]) {//vc
            UIViewController *vc = [class1 new];
            [self addVC:vc atIndex:index];
        } else if ([class1 isSubclassOfClass:[UIView class]]){//view
            UIView *view = [class1 new];
